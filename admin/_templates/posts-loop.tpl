...
{foreach item=job from=$jobs name=tmp}
	<div id="item{$job.id}" class="row {cycle values='odd,even'}">
		<div class="icons">
			...
			{/if}&nbsp;
				<a href="javascript:void(0);" onclick="Jobber.Delete('{$BASE_URL_ADMIN}delete/', {$job.id});" title="Delete this job"><img src="{$BASE_URL_ADMIN}_templates/img/icon-delete.png" alt="delete" /></a>
		</div>
